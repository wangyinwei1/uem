...
const ParamSettingForm = Form.create()((props) => {
    ...
    return (
        <Form>
            <div className={styles.chunk} style={{ marginTop: 0 }}>
                ...
                <div className={styles.content}>
                    <FormItem>
                        <div className={styles['form-item']}>
                            {getFieldDecorator('apdex', {
                                rules: [
                                    { required: true, message: locale('请输入正整数') },
                                    { pattern: /^[1-9]\d*$/, message: locale('请输入正整数') }
                                ],
                                initialValue: apdex
                            })(<Input />)}
                            <span>ms</span>
                    ...
                    <div className={styles['apdex-warning']}>
                        {locale('设置该应用的Apdex T值，即能让用户满意的页面加载时间的阈值,单位为毫秒，缺省值为2000毫秒。')}
                    </div>
                </div>
            </div>
            ...
            <div className={styles.chunk}>
                <span className={styles.label}>{locale('白名单')}: </span>
                <div className={styles.content}>
                    <FormItem>
                        <div className={styles['form-item']}>
                            {getFieldDecorator('url', {
                                rules: [
                                    ...
                                    { type: 'url', message: locale('请输入正确的域名') }
                                ]
                            })(<Input placeholder={locale("请输入域名")} />)}
                            <button className={styles['common-btn']} onClick={addToWhiteList}>{locale('添加到白名单')}</button>
                        </div>
                    </FormItem>
                    {
                        urls.length > 0 &&
                        <div className={styles['white-list-table']}>
                            <button className={styles['common-btn']} onClick={deleteFromWhiteList}>{locale('删除')}</button>
                            <Table
